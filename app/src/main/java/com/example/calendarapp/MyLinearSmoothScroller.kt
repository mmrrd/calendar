 ...
 */
class MyLinearSmoothScroller(context: Context) : SmoothScroller() {
    ...
    init {
        MILLISECONDS_PER_PX = calculateSpeedPerPixel(context.resources.displayMetrics)
    }
    ...
    override fun onSeekTargetStep(dx: Int, dy: Int, state: RecyclerView.State, action: Action) {
        ...
        check(!(DEBUG && mTargetVector != null && (mTargetVector!!.x * dx < 0 || mTargetVector!!.y * dy < 0))) {
    ...
    protected val horizontalSnapPreference: Int
         ...
         * @see .SNAP_TO_ANY
         */
        protected get() = if (mTargetVector == null || mTargetVector!!.x == 0f) SNAP_TO_ANY else if (mTargetVector!!.x > 0) SNAP_TO_END else SNAP_TO_START
    protected val verticalSnapPreference: Int
         ...
         * @see .SNAP_TO_ANY
         */
        protected get() = if (mTargetVector == null || mTargetVector!!.y == 0f) SNAP_TO_ANY else if (mTargetVector!!.y > 0) SNAP_TO_END else SNAP_TO_START
    ...
    override fun computeScrollVectorForPosition(targetPosition: Int): PointF? {
        ...
        if (layoutManager is ScrollVectorProvider) {
            return (layoutManager as ScrollVectorProvider)
                    .computeScrollVectorForPosition(targetPosition)
        }
        Log.w(TAG, "You should override computeScrollVectorForPosition when the LayoutManager"
                + " does not implement " + ScrollVectorProvider::class.java.canonicalName)
        return null
    }
    ...
    companion object {
        ...
        private const val TAG = "LinearSmoothScroller"
